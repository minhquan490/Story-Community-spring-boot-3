package com.story.community.core.common.configuration;
...
/**
 * Clone of {@code SslServerCustomizer} see it for information
 * 
 * @see {@link SslServerCustomizer}
 */
@RequiredArgsConstructor
public class CustomNettyServerCustomizer implements NettyServerCustomizer {
    ...
    /**
     * A {@link KeyManagerFactory} that allows a configurable key alias to be used.
     * Due to
     * the fact that the actual calls to retrieve the key by alias are done at
     * request
     * time the approach is to wrap the actual key managers with a
     * {@link ConfigurableAliasKeyManager}. The actual SPI has to be wrapped as well
     * due
