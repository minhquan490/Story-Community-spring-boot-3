...
public final class HttpConversionUtil {
    /**
     * The set of headers that should not be directly copied when converting headers
     * from HTTP to HTTP/3.
     */
    ...
    /**
     * <a href="https://tools.ietf.org/html/rfc7540#section-8.1.2.3">[RFC 7540],
     * 8.1.2.3</a> states the path must not
    ...
    /**
     * Provides the HTTP header extensions used to carry HTTP/3 information in HTTP
     * objects
     */
    public enum ExtensionHeaderNames {
        /**
         * HTTP extension header which will identify the stream id from the HTTP/3
         * event(s) responsible for
        ...
        /**
         * HTTP extension header which will identify the scheme pseudo header from the
         * HTTP/3 event(s) responsible for
         * generating an {@code HttpObject}
         * <p>
         * {@code "x-http3-scheme"}
         */
        ...
        /**
         * HTTP extension header which will identify the path pseudo header from the
         * HTTP/3 event(s) responsible for
         * generating an {@code HttpObject}
         * <p>
         * {@code "x-http3-path"}
        ...
        PATH("x-http3-path"),
        /**
         * HTTP extension header which will identify the stream id used to create this
         * stream in an HTTP/3 push promise
         * frame
         * <p>
    ...
    /**
     * Apply HTTP/3 rules while translating status code to
     * {@link HttpResponseStatus}
     *
     * @param status The status from an HTTP/3 frame
     * @return The HTTP/1.x status
     * @throws Http3Exception If there is a problem translating from HTTP/3 to
     *                        HTTP/1.x
     */
     ...
     * @param http3Headers        The initial set of HTTP/3 headers to create the
     *                            response with
     * @param alloc               The {@link ByteBufAllocator} to use to generate
     *                            the content of the message
     * @param validateHttpHeaders
     *                            <ul>
     *                            <li>{@code true} to validate HTTP headers in the
     *                            http-codec</li>
     *                            <li>{@code false} not to validate HTTP headers in
     *                            the http-codec</li>
     *                            </ul>
    ...
    static FullHttpRequest toFullHttpRequest(long streamId, Http3Headers http3Headers, ByteBufAllocator alloc,
            boolean validateHttpHeaders) throws Http3Exception {
        ByteBuf content = alloc.buffer();
        // HTTP/3 does not define a way to carry the version identifier that is included
        // in the HTTP/1.1 request line.
     ...
     * @param destinationMessage The object which will contain the resulting
     *                           HTTP/1.x headers.
     * @param addToTrailer       {@code true} to add to trailing headers.
     *                           {@code false} to add to initial headers.
     * @throws Http3Exception If not all HTTP/3 headers can be translated to
     *                        HTTP/1.x.
     */
     ...
     * @param streamId      The stream associated with {@code sourceHeaders}.
     * @param inputHeaders  The HTTP/3 headers to convert.
     * @param outputHeaders The object which will contain the resulting HTTP/1.x
     *                      headers..
     * @param httpVersion   What HTTP/1.x version {@code outputHeaders} should be
     *                      treated as when doing the conversion.
     * @param isTrailer     {@code true} if {@code outputHeaders} should be treated
     *                      as trailing headers.
     *                      {@code false} otherwise.
     * @param isRequest     {@code true} if the {@code outputHeaders} will be used
     ...
     * The following headers are only used if they can not be found in from the
     * {@code HOST} header or the
     * {@code Request-Line} as defined by
     * <a href="https://tools.ietf.org/html/rfc7230">rfc7230</a>
     * <ul>
     * <li>{@link ExtensionHeaderNames#SCHEME}</li>
     * </ul>
     * {@link ExtensionHeaderNames#PATH} is ignored and instead extracted from the
     * {@code Request-Line}.
     */
    ...
    private static CharSequenceMap<AsciiString> toLowercaseMap(Iterator<? extends CharSequence> valuesIter,
        ...
        while (valuesIter.hasNext()) {
            ...
            try {
                ...
                // This is not expect to happen because FIND_COMMA never throws but must be
                // caught
     ...
     * <a href=
     * "https://quicwg.org/base-drafts/draft-ietf-quic-http.html#section-4.1.1">
    ...
    static void toHttp3Headers(HttpHeaders inHeaders, Http3Headers out) {
        ...
        // Choose 8 as a default size because it is unlikely we will see more than 4
        // Connection headers values, but
        // still allowing for "enough" space in the map to reduce the chance of hash
        // code collision.
        CharSequenceMap<AsciiString> connectionBlacklist = toLowercaseMap(
                inHeaders.valueCharSequenceIterator(CONNECTION), 8);
        while (iter.hasNext()) {
            ...
            if (!HTTP_TO_HTTP3_HEADER_BLACKLIST.contains(aName) && !connectionBlacklist.contains(aName)) {
                // https://quicwg.org/base-drafts/draft-ietf-quic-http.html#section-4.1.1 makes
                // a special exception
                // for TE
                if (aName.contentEqualsIgnoreCase(TE)) {
                    ...
                    try {
                        ...
                        // This is not expect to happen because FIND_SEMI_COLON never throws but must be
                        // caught
                        // because of the ByteProcessor interface.
                        throw new IllegalStateException(e);
                    }
     ...
     * <a href=
     * "https://quicwg.org/base-drafts/draft-ietf-quic-http.html#section-4.1.1.1">HTTP3
     * spec</a>.
     */
    ...
    private static final class Http3ToHttpHeaderTranslator {
         ...
         *
         * @param output  The HTTP/1.x headers object to store the results of the
         *                translation
         * @param request if {@code true}, translates headers using the request
         *                translation map. Otherwise uses the
    ...
    private static Http3Exception streamError(long streamId, Http3ErrorCode error, String msg, Exception cause) {
